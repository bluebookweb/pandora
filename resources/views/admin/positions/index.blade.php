...
@section('content')
    <div class="d-flex justify-content-end mb-4">
        <a href="{{ route('position.create') }}" class="btn bg-gradient-primary m-0 ms-2">Add position</a>
    </div>
    <div class="card">
        @if ($products->total())
            <div class="table-responsive">
                <table class="table align-items-center mb-0">
                    ...
                    @foreach($products as $item)
                        <tr>
                            <td>
                                <a href="{{ route('position.edit', $item) }}"
                                   class="text-xs font-weight-bold mb-0">{{ $item->name }}</a>
                            </td>
                            <td>
                                <a href="{{ route('position.edit', $item) }}" class="text-xs font-weight-bold mb-0">
                                    @if (empty($item->image))
                            ...
                            <span
                                class="badge badge-sm @if($item->has_gable == 1) badge-success @else badge-danger @endif">has gable</span>
                            </td>
                            <td class="align-middle text-center text-sm">
                            <span
                                class="badge badge-sm @if($item->has_impact == 1) badge-success @else badge-danger @endif">has impact</span>
                            </td>
